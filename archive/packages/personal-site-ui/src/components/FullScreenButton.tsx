...
class FullScreenButtonRenderer implements CanvasRenderer {
  ...
  constructor(private cnv: HTMLCanvasElement) {
    ...
    const outerRect = stage.createShape(Rectangle, {
      fill: new Color(75, 55, 55, 0.3),
      width: 50,
      height: 35,
      layer: 0,
    });
    const innerRect = stage.createShape(Rectangle, {
      fill: new Color(75, 55, 55, 0.6),
      width: 40,
      height: 25,
      layer: 1,
    });
...
export function FullScreenButton(props: Props) {
  return (
    <button onClick={props.onClick} className={"fullscreen-button"}>
      <CanvasComponent
        renderer={FullScreenButtonRenderer}
