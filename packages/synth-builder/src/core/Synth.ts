...
export function startingGraph() {
  ...
  const gain = new MutableAudioNode(NodeTypes.Gain, { maxGain: 0.12 }, []);
  const analyzer = new MutableAudioNode(
    NodeTypes.Analyser,
    {
      minDecibels: -40,
      fftSize: 1024,
  ...
  const dest = new MutableAudioNode(NodeTypes.AudioDestination, {}, []);
  return MutableAudioGraph.create(
    osc.connect(gain.connect(analyzer.connect(dest))),
  );
}
