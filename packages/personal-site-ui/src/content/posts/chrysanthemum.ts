...
function getSecondLocation(point: DirectionalMagnitude, radius: number) {
  const now = new Date();
  const minutePart =
    Math.PI * 2 * ((now.getSeconds() * 1000 + now.getMilliseconds()) / 60000) +
    Math.PI / 2;
  return {
...
function getMinuteLocation(point: DirectionalMagnitude, radius: number) {
  ...
  const percentOfMinutes =
    (Math.PI * 2 * (secondPart + minutePart)) / (60 * 60) + Math.PI / 2;
...
function getHourLocation(point: DirectionalMagnitude, radius: number) {
  ...
  const percentOfMinutes =
    (Math.PI * 4 * (minutePart + hourPart)) / (60 * 24) + Math.PI / 2;
  ...
  public startMeandering = () => {
    if (this.isMeandering) {
      ...
      const animation = this.stage
        ...
        .transition(
          "cp1",
          { 0: this.segment.cp1, [time]: cp1 },
          TimingFunction.EaseInOutCubic,
        )
        .transition(
          "cp2",
          { 0: this.segment.cp2, [time]: cp2 },
          TimingFunction.EaseInOutCubic,
        )
