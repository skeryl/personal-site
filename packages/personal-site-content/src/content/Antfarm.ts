...
class Wanderer {
  ...
  constructor(
    ...
    this.circle = this.stage.createShape(Circle, {
  ...
  private startWandering = () => {
    if (this.isWandering) {
        ...
        x: Math.max(
          ...
          Math.min(
            ...
            this.currentLocation.x +
              Math.random() * this.stepSize * this.direction.x,
          ),
        ),
        y: Math.max(
          0,
          Math.min(
            ...
            this.currentLocation.y +
              Math.random() * this.stepSize * this.direction.y,
          ),
        ),
      };
      ...
      if (
        this.currentLocation.x >= this.bounds.x ||
        this.currentLocation.x - this.startingRadius <= 0
      ) {
        this.direction.x = getSpecificDirection();
      }

      if (
        this.currentLocation.y >= this.bounds.y ||
        this.currentLocation.y - this.startingRadius <= 0
      ) {
  ...
  digest = () => {
    ...
    if (this.food <= 0) {
      ...
      const expectedRadius =
        this.food > birthCost / 2
          ? this.startingRadius * 1.5
          : this.startingRadius;
  ...
  start = (stage: Stage) => {
    ...
    const onLocationUpdate = (wanderer: Wanderer) => {
      const row = Math.max(
        ...
        Math.min(
          this.numRows - 1,
          Math.floor((wanderer.currentLocation.y - spacing) / outerSquareSize),
        ),
      );
      const col = Math.max(
        0,
        Math.min(
          this.numCols - 1,
          Math.floor((wanderer.currentLocation.x - spacing) / outerSquareSize),
        ),
      );
      ...
      if (
        !nearestRectangle ||
        !nearestRectangle.withinBounds(wanderer.currentLocation)
      ) {
        return;
      }
