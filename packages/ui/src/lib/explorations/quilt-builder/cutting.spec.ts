...
describe('cuttingListFor with composition', () => {
	...
	it('scales blank size by the composition, so a 2x2 halves it', () => {
		...
		// The same pinwheel inside each quarter of a 2x2 gives 3½" blanks.
		...
		expect(nested.rows.map((r) => r.label)).toEqual(['3½”']);
	});
