<script lang="ts">
  ...
  function buildCar(bodyLen: number, bodyW: number, bodyH: number, isHead: boolean): THREE.Group {
    ...
    const silverBody = new THREE.MeshStandardMaterial({
      color: 0xc0c0c4, metalness: 0.75, roughness: 0.25,
    });
    const darkMetal = new THREE.MeshStandardMaterial({
      color: 0x222222, metalness: 0.9, roughness: 0.5,
    });
    ...
    const roof = new THREE.Mesh(roofGeo, new THREE.MeshStandardMaterial({
      color: 0xb0b0b4, metalness: 0.6, roughness: 0.35,
    }));
    ...
    const ridgeMat = new THREE.MeshStandardMaterial({
      color: 0xa0a0a4, metalness: 0.7, roughness: 0.3,
    });
    for (let i = -2; i <= 2; i++) {
      const ridge = new THREE.Mesh(
        new THREE.BoxGeometry(0.06, 0.03, bodyLen * 0.95),
        ridgeMat
      );
    ...
    const acMat = new THREE.MeshStandardMaterial({ color: 0x888888, metalness: 0.7, roughness: 0.35 });
    for (const zOff of [-bodyLen * 0.25, bodyLen * 0.25]) {
      const ac = new THREE.Mesh(
        new THREE.BoxGeometry(bodyW * 0.4, 0.1, bodyLen * 0.25),
        acMat
      );
    ...
    const sideMat = new THREE.MeshStandardMaterial({
      color: 0xb8b8bc, metalness: 0.8, roughness: 0.2,
    });
    for (const xSign of [-1, 1]) {
      const side = new THREE.Mesh(
        new THREE.BoxGeometry(0.02, bodyH * 0.55, bodyLen),
        sideMat
      );
    ...
    const winStripMat = new THREE.MeshStandardMaterial({ color: 0x181828, metalness: 0.1, roughness: 0.6 });
    const paneMat = new THREE.MeshStandardMaterial({
      color: 0x2a3a55, metalness: 0.3, roughness: 0.15,
      emissive: 0x1a2840, emissiveIntensity: 0.4,
    });
    ...
    for (const xSign of [-1, 1]) {
      ...
      for (let i = -5; i <= 5; i++) {
        const pane = new THREE.Mesh(
          new THREE.BoxGeometry(0.03, bodyH * 0.18, 1.0),
          paneMat
        );
      ...
      const stripe = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({
          color: G_GREEN, emissive: G_GREEN, emissiveIntensity: 0.2,
        })
      );
    ...
    if (isHead) {
      ...
      const plate = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({ color: G_GREEN, emissive: G_GREEN, emissiveIntensity: 0.6 })
      );
      ...
      const ws = new THREE.Mesh(
        ...
        new THREE.MeshStandardMaterial({
          color: 0x1a2a40, metalness: 0.2, roughness: 0.1,
    ...
    for (const zPos of [bodyLen * 0.3, -bodyLen * 0.3]) {
      const truck = new THREE.Mesh(
        new THREE.BoxGeometry(bodyW * 1.02, 0.12, 0.55),
        darkMetal
      );
