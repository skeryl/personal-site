...
class OrbitSimulation implements StageContent {
	...
	start = (stage: Stage) => {
		...
		const center = environment.createParticle(
			...
			stage
				.createShape(Circle)
				.setRadius(Math.ceil(size.width / 50))
				.setColor(orange)
		);
		...
		this.stage.onMouseUpdate((mouse) => {
			...
			if (centerCircle.withinBounds(mouse.position())) {
				const start = subtract(centerCircle.position, {
		...
		this.intervalId = window.setInterval(() => {
			const particleGhost = this.createParticle(stage, particleRadius).setPosition(
				particle.position
			);
			const animation = stage
				.animate(particleGhost)
	...
	private createParticle(stage: Stage, particleRadius: number): Circle {
		return stage.createShape(Circle).setRadius(particleRadius).setColor(blue) as Circle;
	}
