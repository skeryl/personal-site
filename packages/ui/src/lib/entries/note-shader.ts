...
import { DefaultKeyMapping } from '@sc/synth-builder/src/model/notes';
import { SynthController } from '@sc/synth-builder/src/core/SynthController';
...
function getTimeDomainDataAsArray(sourceData: Float32Array): Uint8Array {
	return Uint8Array.from(sourceData, (v) => Math.floor(Math.max(0.0, Math.min(255.0, v * 255.0))));
}
...
class NoteShaderContent extends BookOfShadersContent {
	...
	private readonly synthController = new SynthController(this.synth, DefaultKeyMapping);
