...
import { add, Circle, Color, type DirectionalMagnitude, MouseInfo, Path, Stage } from 'grraf';
...
class Pen {
	...
	public addPoint(coordinates: DirectionalMagnitude) {
		...
		if (this.currentLine) {
			...
			this.currentLine = this.stage
				.createShape(Path, { position: coords, fill: this.color, layer: 2 })
...
class FollowContent implements StageContent {
	...
	start = (stage: Stage) => {
		...
		this.pens = [new Pen(this.thickness.current, this.stage)];
	...
	private drawNext = async (
		pos: DirectionalMagnitude,
		count: number
	): Promise<DirectionalMagnitude> => {
		await waitFor(Math.log(count));

		const delta = {
			x: -Math.sin(count) * count * 2,
			y: Math.cos(count) * -2 * count
		};
