...
class AnchoredSquiggle extends Shape {
	private path: Path;
	private target: DirectionalMagnitude = {
		x: this.x + SEGMENT_LENGTH,
		y: this.y
	};
	...
	constructor(stage: Stage, id: number, props: Partial<ShapeProperties>) {
		...
		this.path = this.stage.createShape(Path).setLineCap('round');
	}
...
class SomethingPretty implements StageContent {
	...
	start = (stage: Stage) => {
		...
		while (next !== null) {
			...
			const squiggle = this.stage
				.createShape(AnchoredSquiggle)
		...
		if (this.stage) {
			...
			if (border && this.mouse) {
				...
				this.squiggles.forEach((squiggle) => {
					const progressFromCenter = Math.abs(position.y - (border.y + border.height / 2));

					squiggle
						.setTarget(Math.ceil(position.x), Math.ceil(position.y))
