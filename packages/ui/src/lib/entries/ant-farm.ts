...
function getColor(seed: number): Color {
	return seed in numColors
		? numColors[seed]
		: new Color(Math.random() * 125 + 130, Math.random() * 55 + 85, Math.random() * 125 + 130, 1);
}
	...
	digest = () => {
		...
		if (this.food <= 0) {
			...
			const expectedRadius =
				this.food > birthCost / 2 ? this.startingRadius * 1.5 : this.startingRadius;
