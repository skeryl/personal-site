...
import { Circle, Color, type DirectionalMagnitude, Stage, TimingFunction } from 'grraf';
import { WackySegment, type WackySegmentProps } from '$lib/shapes';
...
function getSecondLocation(point: DirectionalMagnitude, radius: number) {
	const now = new Date();
	const minutePart =
		Math.PI * 2 * ((now.getSeconds() * 1000 + now.getMilliseconds()) / 60000) + Math.PI / 2;
	return {
...
class MeanderingPath {
	...
	constructor(
		...
		this.segment = stage
			.createShape<WackySegment, WackySegmentProps>(WackySegment, {
				a,
				b,
				fill,
				cp1: midpoint,
		...
		const circleRadius = Math.round(
			Math.pow(Math.min(size.width, size.height) / this.radius, 1.6) + 2
		);
		...
		setTimeout(this.tick, untilNextSecond);*/
	}
	...
	public tick = () => {
		if (this.isMeandering) {
			...
			/*this.stage
				...
				.transition(
					'position',
					{ 0: this.circle.position, [time]: cp1 },
			...
			//animation.start();
			this.segment.cp1 = cp1;
			this.segment.cp2 = cp2;
		}
	};
}
...
class ChrysanthemumContent implements StageContent {
	...
	private allPaths: MeanderingPath[] = [];
		...
		for (let i = 0; i < 30; i++) {
			...
			this.allPaths.push(
				new MeanderingPath(
					...
					bounds,
					{
						x: halfWidth + Math.cos(angle) * secondHandSize,
						y: halfHeight - Math.sin(angle) * secondHandSize
					},
					{
						x: halfWidth - Math.cos(angle) * secondHandSize,
						y: halfHeight + Math.sin(angle) * secondHandSize
		...
		for (let i = 0; i < 6; i++) {
			...
			this.allPaths.push(
				new MeanderingPath(
					...
					bounds,
					{
						x: halfWidth + Math.cos(angle) * minuteHandSize,
						y: halfHeight - Math.sin(angle) * minuteHandSize
					},
					{
						x: halfWidth - Math.cos(angle) * minuteHandSize,
						y: halfHeight + Math.sin(angle) * minuteHandSize
		...
		for (let i = 0; i < 6; i++) {
			...
			this.allPaths.push(
				new MeanderingPath(
					...
					bounds,
					{
						x: halfWidth + Math.cos(angle) * hourHandSize,
						y: halfHeight - Math.sin(angle) * hourHandSize
					},
					{
						x: halfWidth - Math.cos(angle) * hourHandSize,
						y: halfHeight + Math.sin(angle) * hourHandSize
	...
	startDrawLoop = () => {
		if (this.isPlaying && this.stage) {
			this.allPaths.forEach((path) => path.tick());
			this.stage.draw();
