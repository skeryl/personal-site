...
import { Circle, Color, type DirectionalMagnitude, MouseInfo, Path, Stage } from 'grraf';
...
export class MirrorsContent implements StageContent {
	...
	start = (stage: Stage) => {
		...
		this.pens = [
			...
			...new Array(3).fill(undefined).map(
				(_, ix) =>
					new Pen(this.thickness.current, stage, ({ x, y }) => ({
						x: x,
						y: y + 20 * ix
					}))
			),
		...
		function averageChange(positions: DirectionalMagnitude[]): DirectionalMagnitude {
			...
			type Result = {
				result: DirectionalMagnitude;
				last: DirectionalMagnitude;
			};
			const sum = positions.reduce(
				(result: Result, current: DirectionalMagnitude) => {
					result.result = {
	...
	private setThickness(additional: number) {
		...
		this.thickness.current = Math.min(
			this.thickness.max,
